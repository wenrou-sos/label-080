...
import type { RoomStatus, TechnicianStatus, UserRole } from '@/types'
...
export default function ReceptionPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <Header
        title="足道养生会馆 · 运营控制台"
        subtitle={isAdmin ? '管理员模式' : '前台管理系统'}
        showScreenLink={isAdmin}
      >
        <DatePicker />
      </Header>
        ...
        {isReception && (
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <StatCard
              icon={DoorOpen}
              label="可用房间"
              value={rooms.filter((r) => r.status === 'cleaned').length}
              subValue={'共 ' + rooms.length + ' 间'}
              color="bg-gradient-to-br from-emerald-500 to-teal-600"
            />
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-slate-900/50 rounded-2xl border border-slate-800 overflow-hidden">
              ...
              <div className="divide-y divide-slate-800/50 max-h-[500px] overflow-y-auto">
                ...
                {queue.length === 0 && (
                  <div className="p-12 text-center text-slate-500 text-sm">暂无排队顾客</div>
                )}
              </div>
            </div>
